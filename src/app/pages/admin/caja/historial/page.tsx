...
const hoy = new Date();
const mesActual = String(hoy.getMonth() + 1); // 1..12
...
const VerPagos = () => {
  ...
  const cargarDatosIniciales = async () => {
    try {
      setLoading(true);
      setError("");
      // Carga catálogos en paralelo
      await Promise.all([cargarClientes(), cargarMetodosPago()]);
      // Carga pagos del mes/año actual por defecto
      await cargarPagosPorMes(parseInt(filtros.mes, 10), parseInt(filtros.anio, 10));
    } catch (error) {
  ...
  const aplicarFiltros = () => {
    ...
    // Prioriza filtro por mes/año si están ambos
    if (mes && anio) {
      cargarPagosPorMes(parseInt(mes, 10), parseInt(anio, 10));
      return;
    }
  ...
  const limpiarFiltros = () => {
    // Reinicia a mes/año actuales por defecto
    setFiltros({
      cliente: "",
      metodo: "",
      mes: mesActual,
    ...
    cargarPagosPorMes(parseInt(mesActual, 10), parseInt(anioActual, 10));
  };
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md mb-6">
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={aplicarFiltros} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
              Aplicar Filtros
            </button>
            <button
                ...
                {pagosFiltrados.length === 0 ? (
                  ...
                  pagosFiltrados.map((pago) => (
                    <tr key={pago.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-4 text-sm font-semibold text-green-600">{formatMonto(pago.monto)}</td>
                      ...
                      <td className="px-4 py-4 text-sm text-slate-900">{pago.referencia || "-"}</td>
        ...
        {pagosFiltrados.length > 0 && (
          <div className="mt-4 bg-white p-4 rounded-lg shadow-md">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-semibold text-green-600">Total: {formatMonto(totalMonto)}</span>
            </div>
          </div>
        )}
