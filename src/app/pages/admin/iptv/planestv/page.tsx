...
const PlanesTv = () => {
  ...
  const fetchPlanes = async () => {
    try {
      const response = await fetch(`${apiHost}/api/tv/planes`);
      const data = await response.json();
      setPlanes(data);
    } catch (error) {
      console.error("Error al obtener los planes IPTV:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingPlan 
        ? `${apiHost}/api/tv/planes/${editingPlan.id}`
        : `${apiHost}/api/tv/planes`;
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`${apiHost}/api/tv/planes/${id}`, {
        method: "DELETE",
      });
    ...
    <AdminLayout>
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-orange-600">Gestión de Planes</h1>
