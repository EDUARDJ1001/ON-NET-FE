...
const GastosTvPage = () => {
    const [gastos, setGastos] = useState<GastoTv[]>([]);
    ...
    const [editing, setEditing] = useState<GastoTv | null>(null);
    ...
    const cargarGastos = async (): Promise<void> => {
        try {
            ...
            const url = `${apiHost}/api/gastos-tv${params.toString() ? `?${params.toString()}` : ""}`;
            ...
            if (!res.ok) {
                ...
                res = await safeFetch(`${apiHost}/api/gastos-tv`);
                if (res.status === 499) return;
            }

            if (!res.ok) throw new Error("Error al obtener gastos");
            const data: GastoTv[] = await res.json();
    ...
    const guardarGasto = async (): Promise<void> => {
        ...
        try {
            ...
            if (modalMode === "create") {
                const res = await safeFetch(`${apiHost}/api/gastos-tv`, { method: "POST", body });
                if (res.status === 499) return;
                if (!res.ok) throw new Error("No se pudo crear el gasto");
            } else if (modalMode === "edit" && editing) {
                const res = await safeFetch(`${apiHost}/api/gastos-tv/${editing.id}`, { method: "PUT", body });
                if (res.status === 499) return;
    ...
    const eliminarGasto = async (g: GastoTv): Promise<void> => {
        if (!confirm(`¿Eliminar el gasto "${g.descripcion}" por ${formatMonto(g.monto)}?`)) return;
        try {
            setLoading(true);
            const res = await safeFetch(`${apiHost}/api/gastos-tv/${g.id}`, { method: "DELETE" });
            if (res.status === 499) return;
    ...
    const gastosFiltrados = useMemo<GastoTv[]>(() => {
        let list = [...gastos];
        if (filtros.q.trim()) {
    ...
    const visibleRows = useMemo<GastoTv[]>(() => {
        ...
        <AdminLayout>
            <div className="px-4 sm:px-6 lg:px-8 py-16">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                    <div>
                        <h1 className="text-2xl font-bold text-orange-600">Gastos Tv</h1>
                        <p className="text-slate-600 mt-1">Gestione los gastos operativos de IPTV</p>
                    </div>
